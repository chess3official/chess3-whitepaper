# Chessalienz: Pawnz

## Genesis Collection Overview

**Chessalienz: Pawnz** is the genesis NFT collection of the Chess3 ecosystem—5,000 unique hand-designed vector pawn beings from the CHESS3 dimension.

## Collection Specifications

| Attribute | Details |
|-----------|---------|
| **Total Supply** | 5,000 NFTs |
| **Blockchain** | Solana |
| **Mint Price** | 2 SOL |
| **Character Roles** | 21 unique roles |
| **Possible Outcomes** | 112M+ |
| **Ultra-Rare Sets** | 42 matching character combinations |
| **Max Per Transaction** | 10 NFTs |

## The Story

Meet the hand-designed vector pawn beings from the CHESS3 dimension. These aren't just digital collectibles—they're your passport to an interstellar chess r3volution.

Each Chessalienz represents a unique character role in the Chess3 universe, from fierce Fighters to mystical Shamans, from cunning Thieves to noble Commanders. Together, they form an army of chess warriors ready to grant Earthlings access to interstellar P2E chess shenanigans.

## Why "Pawnz"?

In chess, pawns are often underestimated—yet they're the foundation of every strategy. They can transform into any piece upon reaching the other side of the board. Similarly, Chessalienz: Pawnz holders are the foundation of the Chess3 ecosystem, with the potential to unlock unlimited opportunities as the platform grows.

## Rarity System

The Chessalienz: Pawnz collection features a sophisticated 4-tier rarity system:

### Trait System
Each Chessalienz: Pawnz NFT consists of **8 layers**:
1. **Background** (18 colors + 4 special: Hollowblack, Hollowwhite, Shinyblack, Shinywhite)
2. **Color** (White/Black)
3. **Body** (21 character roles)
4. **Head** (21 character roles)
5. **Ring** (21 character roles)
6. **Base** (21 character roles)
7. **Outline**
8. **Foil** (Ultra trait overlay effect)

With 8 layers and multiple traits per layer, there are **112M+ possible outcomes**!

### Ultra Rare (0.84% - 42 NFTs)
- **All matching ultra traits** across body, head, ring, and base
- **Exclusive hollow backgrounds** (Hollowblack or Hollowwhite)
- **Foil layer activated** for maximum visual impact
- **Only 1 per character per color** - the rarest of the rare!
- 21 with Hollowblack background + White color
- 21 with Hollowwhite background + Black color

### Super Rare (1.68% - 84 NFTs)
- **All matching regular traits** across body, head, ring, and base
- **Exclusive shiny backgrounds** (Shinyblack or Shinywhite)
- **2 per character per color** - extremely limited!
- 42 with Shinyblack background + White color
- 42 with Shinywhite background + Black color

### Rare (5% - 250 NFTs)
- **At least 1 ultra trait** mixed with regular traits
- Standard backgrounds
- Unique combinations with premium traits

### Common & Uncommon (92.48% - 4,624 NFTs)
- **Uncommon**: Matching regular traits (all 4 layers same character)
- **Common**: Random trait combinations
- Standard backgrounds and colors
- Wide variety of character combinations

## Utility Beyond Art

While the artwork is stunning, Chessalienz: Pawnz NFTs are functional assets that grant:

✅ **Platform Access** - Early entry to Chess3 beta  
✅ **Tournament Entry** - Exclusive holder competitions  
✅ **Governance Rights** - Vote on platform decisions  
✅ **Staking Rewards** - Earn passive income  
✅ **Reduced Fees** - Lower platform transaction costs  
✅ **Special Airdrops** - Token and NFT rewards  
✅ **VIP Status** - Private community channels  

## Mint Details

### Public Mint
- **Date**: Available Now
- **Price**: 2 SOL per NFT
- **Supply**: 5,000 NFTs total
- **Limit**: 10 per transaction
- **Platform**: Magic Eden
- **Collection Address**: `F7w5sWcchWpCP7TznnSqZFRzreP9PW4gU19eZ4QkKj78`
- **Mint Link**: [Magic Eden Collection](https://magiceden.io/collections/solana/F7w5sWcchWpCP7TznnSqZFRzreP9PW4gU19eZ4QkKj78)

### How to Mint
1. **Visit Magic Eden**: Navigate to the official collection page
2. **Connect Wallet**: Use your Solana wallet (Phantom, Solflare, etc.)
3. **Select Quantity**: Choose how many NFTs to mint (max 10 per transaction)
4. **Approve Transaction**: Confirm the 2 SOL mint price per NFT
5. **Receive NFTs**: Your Chessalienz: Pawnz NFTs appear in your wallet instantly

## Post-Mint Roadmap

### Immediate (Week 1)
- Reveal all NFTs
- Secondary market trading on Magic Eden
- Holder verification in Discord
- Snapshot for future airdrops

### Short-Term (Month 1-3)
- Staking mechanism launch
- First holder-exclusive tournament
- Governance proposals begin
- Platform beta access

### Long-Term (Month 3+)
- Token airdrop to holders
- Expanded utility features
- Breeding/3volution mechanics (potential)
- Additional collection drops

## Investment Perspective

While we can't guarantee returns, Chessalienz: Pawnz holders benefit from:

1. **First-Mover Advantage**: Genesis collection status
2. **Utility Accrual**: Growing benefits as platform develops
3. **Community Building**: Strong, engaged holder base
4. **Long-Term Vision**: Sustainable project roadmap
5. **Fair Distribution**: No team allocation or insider deals

## Redemption Process
1. **Holder Verification**: Connect your wallet to verify NFT ownership
2. **Claim Period**: Limited-time windows to claim physical rewards
3. **Shipping Worldwide**: Free global shipping for verified holders
4. **Tier Scaling**: Reward value scales with NFT rarity and holding duration

---

**Ready to join the CHESS3 dimension? Mint your Chessalienz: Pawnz on Magic Eden and become a founding member of the r3volution.**
